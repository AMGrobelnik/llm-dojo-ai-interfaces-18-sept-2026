...
```bash
SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || echo /research-monorepo)/.claude/skills/aii-data-fig-gen"
...
- **No LaTeX-native output.** PGFPlots produces the best camera-ready
  ...
  `<author>/aii_tex`, which `aii_pipeline.bundles.ensure_tex()` fetches at
  `gen_full_paper`. Figure generation runs in the pipeline loop, HOURS
