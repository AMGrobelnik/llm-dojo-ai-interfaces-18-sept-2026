...
from _aii_hf_endpoints import PARQUET_URL
...
def _stream_shards_to_json(client, shards, output_dir: str, base_name: str) -> dict:
    """Stream parquet shards → full/mini/preview JSON without materializing the
    whole split in RAM.

    The previous parquet path held the entire split in memory at once: each
    shard was read with ``r.content`` (full shard buffered), converted with
    ``table.to_pylist()`` (every row as a Python object), and accumulated across
    shards into one ``rows`` list — a multi-GB blow-up for large datasets and,
    at ``max_workers`` concurrent downloads, a server OOM vector. Here the HTTP
    body streams to disk in 1 MiB chunks and the parquet file is read in row
    batches, with each row written to ``full_*.json`` immediately and only the
    first few rows retained for mini/preview. Peak memory ≈ one batch,
    independent of dataset size (mirrors the load_dataset path's chunked write).
    """
    import json
    ...
    with open(full_file, "w", encoding="utf-8") as out:
        ...
        for shard in shards:
            ...
            try:
                with client.stream("GET", shard["url"]) as r:
                    r.raise_for_status()
                    with open(tmp_path, "wb") as fb:
                        for chunk in r.iter_bytes(chunk_size=1 << 20):
                            fb.write(chunk)
                for batch in pq.ParquetFile(tmp_path).iter_batches(batch_size=1000):
    ...
    with open(mini_file, "w", encoding="utf-8") as f:
        json.dump(mini, f, indent=2, ensure_ascii=False, default=str)

    preview_file = Path(output_dir) / f"preview_{base_name}.json"
    with open(preview_file, "w", encoding="utf-8") as f:
        json.dump(
            [_truncate_value(row) for row in mini], f, indent=2, ensure_ascii=False, default=str
        )
    ...
    with httpx.Client(headers=headers, timeout=CONNECTION_TIMEOUT, follow_redirects=True) as client:
        for split_name, shards in by_split.items():
            try:
                ...
                info = _stream_shards_to_json(client, shards, output_dir, base_name)
...
def core_download_dataset(
    ...
    Args:
        ...
        revision: Git revision of the dataset repo to load (a commit SHA pins
...
def main():
    ...
    try:
        ...
        result = call_server(SERVER_NAME, params, timeout=180.0)
    except Exception:
        result = None
    ...
    if result is None:
        ...
        result = core_download_dataset(**params)
