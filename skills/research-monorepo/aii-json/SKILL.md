...
```
export SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || echo /research-monorepo)/.claude/skills/aii-json"
...
```bash
SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || echo /research-monorepo)/.claude/skills/aii-json" && \
...
- **Always pass an absolute path.** Relative paths resolve from the
  ability server's CWD (typically ``/research-monorepo/aii_server``), not from
