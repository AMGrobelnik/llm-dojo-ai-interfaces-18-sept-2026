---
name: aii-lean
description: "Compiles and verifies Lean 4 proofs against Mathlib with aii_run_lean.py, tries closing tactics at every sorry position with aii_lean_suggest.py, and searches Mathlib by type pattern through Loogle with aii_mathlib_pattern_search.py, all runnable in parallel. Use whenever a theorem, lemma, or proof has to be formalized or machine-checked, a Lean file fails to compile, a sorry needs filling, or an existing Mathlib lemma has to be located. Triggers: Lean 4, Mathlib, .lean file, theorem, lemma, proof obligation, sorry-driven development, tactics such as exact?, apply?, simp?, ring, linarith, omega, aesop, Loogle query, formal verification, checking autoformalization output. NOT for: neuro-symbolic or LLM-plus-solver research framing and study design (use aii-handbook-auto-neurosymbolic), SMT, Prolog or ASP solvers, or general Python scripting conventions (use aii-python)."
---

**IMPORTANT - Path resolution:** Always use an absolute SKILL_DIR. The CWD may not be the project root (e.g. on worker pods).
```
export SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || echo /research-monorepo)/.claude/skills/aii-lean"
...
```bash
SKILL_DIR="$(git rev-parse --show-toplevel 2>/dev/null || echo /research-monorepo)/.claude/skills/aii-lean" && \
