...
def init_owid_download():
    """Silence tqdm so progress bars don't pollute the ability's output.

    Was also documented as pre-importing heavy dependencies and warming
    imports. It did neither: both comments sat above an empty
    ``try: pass / except Exception: pass``, so the only thing this function
    ever did was set the env var. The warmup body was removed at some point
    and the scaffolding stayed, leaving comments that described behaviour the
    code did not have.
    """
    import os
...
def main():
    ...
    if result is None:
        ...
        init_owid_download()
        result = core_owid_download(**params)
