...
def core_semscholar_bib_fetch(references: list | None = None) -> dict:
    ...
    # Tolerate the common ``{"references": [...]}`` wrapper a caller may send
    # instead of a bare list — unwrap it rather than iterate the dict's (string)
    # keys and crash on ``ref.get(...)`` with AttributeError.
    if isinstance(references, dict):
        inner = references.get("references")
        references = inner if isinstance(inner, list) else []
    if not isinstance(references, list):
        return {
            "success": False,
            "error": f"references must be a list of objects, got {type(references).__name__}",
        }
    ...
    for i, ref in enumerate(references):
        if not isinstance(ref, dict):
            # A stray non-dict entry (e.g. a bare string) — skip it rather than
            # crash the whole batch on ``.get()``.
            continue
        # ``or ""`` guards an explicit ``null`` value (``.get(k, "")`` only
        # defaults on a MISSING key, not a present-but-null one).
