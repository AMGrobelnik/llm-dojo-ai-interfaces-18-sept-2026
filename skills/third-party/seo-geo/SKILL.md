---
name: seo-geo
description: "Optimizes a live website for traditional search (Google, Bing) and for AI search engines (ChatGPT, Perplexity, Gemini, Copilot, Claude): technical audit via seo_audit.py, DataForSEO keyword, SERP, backlink and competitor-gap scripts, JSON-LD schema such as FAQPage, Article and Organization, meta and Open Graph tags, robots.txt access for AI crawlers, and the nine Princeton GEO methods. Use whenever search visibility, ranking, indexing, or getting cited by an AI assistant is the goal. Triggers: SEO, GEO, generative engine optimization, keyword research, search volume, SERP, backlinks, schema markup, JSON-LD, meta description, robots.txt, GPTBot, PerplexityBot, ClaudeBot, AI Overview, E-E-A-T, sitemap, rich results. NOT for: picking, checking or purchasing a domain name, which domain-hunter and amg-is-domain-taken handle; NOT for DNS records, CDN cache or edge analytics, which amg-cloudflare owns."
---
...
|--------|-----------------|--------------|
| **Cite Sources** | +40% | Add citations to authoritative sources |
| **Statistics Addition** | +37% | Include specific numbers and data |
